...
function hablar(texto, callback) {
  ...
  utterance.onstart = () => {
    personaje.setAttribute("src", "talk.json");
  };
...
function mostrarPregunta() {
  ...
  preguntaTexto.textContent = `${indicePregunta + 1}. ${preguntaActual.pregunta}`;
...
function verificarRespuesta(indiceSeleccionado) {
  ...
  const indiceCorrecto = letraCorrecta.charCodeAt(0) - 65;
  const esCorrecto = indiceSeleccionado === indiceCorrecto;

  const respuestaTexto = esCorrecto
    ? respuestasCorrectasAleatorias()
    : respuestasIncorrectasAleatorias();
...
function mostrarResultado() {
  ...
  if (puntuacion > 25) {
    hablar("¡Increíble! Has aprobado, a bailar como un mono feliz.", () => {
      ...
      const segundoBailarín = document.createElement("lottie-player");
      segundoBailarín.setAttribute("src", "dancesin.json");
      segundoBailarín.setAttribute("autoplay", "");
      segundoBailarín.setAttribute("loop", "");
      segundoBailarín.style.width = "200px";
      segundoBailarín.style.height = "200px";

      resultadoContainer.appendChild(segundoBailarín);
    });
  } else {
